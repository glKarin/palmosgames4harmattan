...
#else
		QString p = QFileInfo(TAR_APP).absolutePath();
		QString np = QString("PATH=%1:$PATH").arg(p);
		env << np;
    env.replaceInStrings(QRegExp("^PATH=(.*)", Qt::CaseInsensitive), np);
    _proc1.setEnvironment(env);
		//qDebug()<<_proc1.environment();
#endif
...
#else
#ifdef _DBG
		qDebug() << QString("/usr/bin/dpkg-deb --fsys-tarfile \"" + _game->IPKPath() + "\"" " | " TAR_APP" x --wildcards \"*appinfo.json\" -O");
#endif
    connect(&_proc2, SIGNAL(finished(int)), this, SLOT(Step2Finished()));
    _proc1.start("/usr/bin/dpkg-deb --fsys-tarfile \"" + _game->IPKPath() + "\"");
    _proc2.start(TAR_APP" x --wildcards \"*appinfo.json\" -O");
