...
void QMaemo5ValueButton::setValueText(const QString &text)
{
	if(sValueText != text)
	{
		sValueText = text;
		SetupRenderText();
		emit valueTextChanged(sValueText);
	}
}
...
void QMaemo5ValueButton::setValueLayout(ValueLayout_e l)
{
	Q_UNUSED(l);
}

void QMaemo5ValueButton::setText(const QString &text)
{
	if(sText != text)
	{
		sText = text;
		QPushButton::setText(sText);
	}
}

QString QMaemo5ValueButton::text() const
{
	return sText;
}

void QMaemo5ValueButton::SetupRenderText()
{
	QString text = sText + "\n" + sValueText;
