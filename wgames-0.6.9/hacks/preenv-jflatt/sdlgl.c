...
#if 0
#define HARMATTAN_MEM 1008108 * 1024
#define _TRACE TRACE

#define CHECK_POINTER(p) (p && ((uintptr_t)p < HARMATTAN_MEM))

void sincosl(long double x, long double *sin, long double *cos)
{
	OVERRIDES(sincosl, (long double x, long double *sin, long double *cos), void);
	long double sx, sy;
	_TRACE("Override: %s(%p, %p[%d], %p[%d]) -> %p\n", __func__, super_func, sin, CHECK_POINTER(sin), cos, CHECK_POINTER(cos), sincosl);
	SUPER(x, &sx, &sy);
	if (CHECK_POINTER(sin)) *sin = sx;
	if (CHECK_POINTER(cos)) *cos = sy;
}

void sincosf(float x, float *sin, float *cos)
{
	OVERRIDES(sincosf, (float x, float *sin, float *cos), void);
	float sx, sy;
	_TRACE("Override: %s(%p, %p[%d], %p[%d]) -> %p\n", __func__, super_func, sin, CHECK_POINTER(sin), cos, CHECK_POINTER(cos), sincosf);
	SUPER(x, &sx, &sy);
	if (CHECK_POINTER(sin)) *sin = sx;
	if (CHECK_POINTER(cos)) *cos = sy;
}
...
{
	OVERRIDES(sincos, (float x, float *sin, float *cos), void);
	float sx, sy;
	_TRACE("Override: %s(%p, %p[%d], %p[%d]) -> %p\n", __func__, super_func, sin, CHECK_POINTER(sin), cos, CHECK_POINTER(cos), sincos);
	SUPER(x, &sx, &sy);
	if (CHECK_POINTER(sin)) *sin = sx;
	if (CHECK_POINTER(cos)) *cos = sy;
}
...
#if 0
void glBindTexture__(GLenum mode, GLuint t)
{
	OVERRIDES(glBindTexture, (GLenum e, GLuint t), void);
	SUPER(mode, t);
}

void glDrawArrays__(GLenum m, GLint s, GLsizei c)
{
	GLuint t;
	...
	if(1)
	{
		...
		glLoadIdentity();
		glOrthof(0, 480, -480, 0, -1, 1);
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
	}
	glClearColor(0,0,0,1);
	OVERRIDES(glDrawArrays, (GLenum m, GLint s, GLsizei c), void);
...
#undef R
	...
	SUPER(m, s, c);
	//Test_RenderTexture(0);
}




struct idOpenGLState_s
{
	GLenum matrix_mode;
	GLfloat pm[16];
	GLfloat mvm[16];
	...
	struct idVertexArray_s va;
	struct idVertexArray_s ta;
};

void Test_DumpState(struct idOpenGLState_s *s)
{
	if(!s)
		return;

	glGetIntegerv(GL_MATRIX_MODE, (GLint *)&s->matrix_mode);
	glGetFloatv(GL_PROJECTION_MATRIX, s->pm);
	glGetFloatv(GL_MODELVIEW_MATRIX, s->mvm);

	s->tex_enabled = glIsEnabled(GL_TEXTURE_2D);
	glGetIntegerv(GL_TEXTURE_BINDING_2D, (GLint *)&s->tex_binded);

#define VA(T, obj)  \
	glGetIntegerv(GL_##T##_ARRAY_BUFFER_BINDING, (GLint *)&obj.buffered); \
	glGetIntegerv(GL_##T##_ARRAY_SIZE, (GLint *)&obj.size); \
	glGetIntegerv(GL_##T##_ARRAY_STRIDE, (GLint *)&obj.stride); \
	glGetIntegerv(GL_##T##_ARRAY_TYPE, (GLint *)&obj.type); \
	glGetPointerv(GL_##T##_ARRAY_POINTER, &obj.pointer); \
	obj.enabled = glIsEnabled(GL_##T##_ARRAY);
	VA(VERTEX, s->va)
	VA(TEXTURE_COORD, s->ta)
#undef VA
}

void Test_RestoreState(const struct idOpenGLState_s *s)
{
	if(!s)
		return;
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadMatrixf(s->mvm);
	glMatrixMode(s->matrix_mode);

	{
		OVERRIDES(glBindTexture, (GLenum e, GLuint t), void);
		SUPER(GL_TEXTURE_2D, s->tex_binded);
		//glBindTexture(GL_TEXTURE_2D, s->tex_binded);
	}
	(s->tex_enabled ? glEnable : glDisable)(GL_TEXTURE_2D);

#define VA(T, t, obj) \
	glBindBuffer(GL_ARRAY_BUFFER, obj.buffered); \
	gl##t##Pointer(obj.size, obj.type, obj.stride, obj.pointer); \
	(obj.enabled ? glEnableClientState : glDisableClientState)(GL_##T##_ARRAY);

	VA(VERTEX, Vertex, s->va)
	VA(TEXTURE_COORD, TexCoord, s->ta)
#undef VA
}

void Test_RenderTexture(GLuint tex)
{
	GLuint t;
	struct idOpenGLState_s state;
	...
	if(!glIsTexture(t))
	{
		printf("Texture ID is not texture.\n");
		return;
	}
	
	Test_DumpState(&state);
	{
		GLfloat vs[] = {
			0, 0,
			480, 0,
		...
		OVERRIDES(glDrawElements, (GLenum m, GLsizei c, GLenum e, const void* i), void);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		SUPER(GL_TRIANGLE_STRIP, 4, GL_UNSIGNED_SHORT, is);
		glFinish();
	}
	Test_RestoreState(&state);
}

void Test_PrintTextureData(GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid *data)
{
	int w;
	int i, j, m;
	...
	if(!data)
		return;
	w = format == GL_RGBA ? 4 : (format == GL_RGB ? 3 : 0);
	if(w == 0)
		return;
